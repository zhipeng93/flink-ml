...
public final class FeedbackChannelWithSpill<T> implements Closeable {
    ...
    private final FeedbackQueueWithSpill<T> queue;
    ...
    public void put(T value) {
        if (!queue.addAndCheckIfWasEmpty(value)) {
            // successfully added @value into the queue, but the queue wasn't (atomically) drained
            // yet,
    ...
    private static final class ConsumerTask<T> implements Runnable, Closeable {
        ...
        ConsumerTask(
                Executor executor, FeedbackConsumer<T> consumer, FeedbackQueueWithSpill<T> queue) {
