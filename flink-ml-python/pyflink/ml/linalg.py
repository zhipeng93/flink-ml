...
class VectorTypeInfo(TypeInformation):
    def __init__(self):
        ...
        self._dense_int_double_vector_type_info = DenseIntDoubleVectorTypeInfo()
        self._sparse_int_double_vector_type_info = SparseIntDoubleVectorTypeInfo()
        self._sparse_long_double_vector_type_info = SparseLongDoubleVectorTypeInfo()
    ...
    def to_internal_type(self, obj):
        ...
        if isinstance(obj, DenseIntDoubleVector):
            return chr(0).encode('latin-1') + self._dense_int_double_vector_type_info\
                .to_internal_type(obj)
        elif isinstance(obj, SparseIntDoubleVector):
            return chr(1).encode('latin-1') + self._sparse_int_double_vector_type_info\
                .to_internal_type(obj)
        else:
            return chr(2).encode('latin-1') + self._sparse_long_double_vector_type_info\
                .to_internal_type(obj)

    def from_internal_type(self, obj):
        if obj is not None:
            if obj[0] == 0:
                return self._dense_int_double_vector_type_info.from_internal_type(obj[1:])
            elif obj[0] == 1:
                return self._sparse_int_double_vector_type_info.from_internal_type(obj[1:])
            else:
                return self._sparse_long_double_vector_type_info.from_internal_type(obj[1:])
...
class SparseLongDoubleVectorTypeInfo(TypeInformation):
    def __init__(self):
        super(SparseLongDoubleVectorTypeInfo, self).__init__()
        self._output_stream = OutputStream()
        self._input_stream = InputStream(None)
    ...
    def to_internal_type(self, obj):
        if obj is None:
            return
        assert isinstance(obj, SparseLongDoubleVector)
        stream = self._output_stream
        stream.write_int32(obj.size())
        ...
        stream.write_int32(l)
        for i in range(l):
            stream.write_int64(obj._indices[i])
            stream.write_double(obj._values[i])
        internal_data = bytearray(stream.get())
        stream.clear()
        return internal_data
    ...
    def get_java_type_info(self):
        if not self._j_typeinfo:
            self._j_typeinfo = get_gateway().jvm \
                .org.apache.flink.ml.linalg.typeinfo.SparseLongDoubleVectorTypeInfo.INSTANCE
        return self._j_typeinfo

    def __eq__(self, o) -> bool:
        return isinstance(o, SparseLongDoubleVectorTypeInfo)
...
class Vector(ABC):
    """
    Abstract class for DenseIntDoubleVector, SparseIntDoubleVector and SparseLongDoubleVector.
    """
...
class SparseLongDoubleVector(Vector):
    """
    A sparse vector, using either a dict, a list of (index, value) pairs, or two separate
    arrays of long indices and double values.

    Example:
    ::

        >>> Vectors.sparse(4, {1: 1.0, 3: 5.5})
        SparseLongDoubleVector(4, {1: 1.0, 3: 5.5})
        >>> Vectors.sparse(4, [(1, 1.0), (3, 5.5)])
        SparseLongDoubleVector(4, {1: 1.0, 3: 5.5})
        >>> Vectors.sparse(4, [1, 3], [1.0, 5.5])
        SparseLongDoubleVector(4, {1: 1.0, 3: 5.5})
    ...
    def __init__(self, size: int, *args):
        ...
        if len(args) == 1:
            ...
            if isinstance(pairs, dict):
                pairs = pairs.items()
            pairs = sorted(pairs)
            self._indices = np.array([p[0] for p in pairs], dtype=np.int64)
            self._values = np.array([p[1] for p in pairs], dtype=np.float64)
        else:
            assert len(args[0]) == len(args[1]), "The length of indices and values should be same"
            # two separate arrays of indices and values.
            self._indices = np.array(args[0], dtype=np.int64)
            self._values = np.array(args[1], dtype=np.float64)
            for i in range(len(self._indices) - 1):
                if self._indices[i] >= self._indices[i + 1]:
                    raise TypeError(
    ...
    def set(self, i: int, value: np.float64):
        ...
        if idx < len(self._indices) and self._indices[idx] == i:
            ...
            assert i < self._size
            cur_len = len(self._indices)
            indices = np.zeros(cur_len + 1, dtype=np.int64)
            values = np.zeros(cur_len + 1, dtype=np.float64)
            indices[0:idx] = self._indices[0:idx]  # type: ignore
            values[0:idx] = self._values[0:idx]  # type: ignore
            indices[idx] = i
    ...
    def dot(self, other: Union[Vector, np.ndarray, Sized]) -> np.ndarray:
        ...
        ::
            ...
            >>> sparse = SparseLongDoubleVector(4, [1, 3], [3.0, 4.0])
            >>> sparse.dot(sparse)
            25.0
            >>> sparse.dot(array.array('d', [1., 2., 3., 4.]))
            22.0
            >>> sparse2 = SparseLongDoubleVector(4, [2], [1.0])
            >>> sparse.dot(sparse2)
            0.0
            >>> sparse.dot(np.array([[1, 1], [2, 2], [3, 3], [4, 4]]))
            array([22., 22.])
        ...
        if isinstance(other, SparseLongDoubleVector):
            self_cmind = np.in1d(self._indices, other._indices, assume_unique=True)
            self_values = self._values[self_cmind]
            if self_values.size == 0:
                return np.float_(0.0)  # type: ignore
    ...
    def __repr__(self):
        ...
        vals = self._values
        entries = ", ".join(["{0}: {1}".format(inds[i], float(vals[i])) for i in range(len(inds))])
        return "SparseLongDoubleVector({0}, {{{1}}})".format(self._size, entries)
