...
public interface LogisticRegressionWithFtrlParams<T>
    ...
    Param<Integer> EVALUATE_INTERVAL =
            new IntParam(
                    "evaluationInterval",
                    "Number of iterations to perform one global evaluate.",
                    100,
                    ParamValidators.gtEq(1));
    ...
    default int getEvaluateInterval() {
        return get(EVALUATE_INTERVAL);
    }

    default T setEvaluateInterval(int value) {
        return set(EVALUATE_INTERVAL, value);
    }
}
