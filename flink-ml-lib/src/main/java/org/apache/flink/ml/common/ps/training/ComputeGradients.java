...
public class ComputeGradients extends ProcessStage<MiniBatchMLSession<LabeledPointWithWeight>> {
    ...
    public void process(MiniBatchMLSession<LabeledPointWithWeight> session) throws IOException {
        ...
        double[] gradients =
                computeGradient(session.batchData, Tuple2.of(indices, modelValues), session);
    ...
    private double[] computeGradient(
            ...
            MiniBatchMLSession<LabeledPointWithWeight> session) {
        ...
        for (LabeledPointWithWeight dataPoint : batchData) {
            ...
            double loss = lossFunc.computeLoss(dataPoint.label, dot) * dataPoint.weight;
            totalLoss += loss;
        ...
        session.localLoss[0] = totalWeight > 0 ? totalLoss / totalWeight / session.numWorkers : 0;
        double[] cumGradientValues = new double[modelIndices.length];
        for (int i = 0; i < modelIndices.length; i++) {
            cumGradientValues[i] = cumGradients.get(modelIndices[i]);
        }
        BLAS.scal(1.0 / batchData.size() / session.numWorkers, Vectors.dense(cumGradientValues));
