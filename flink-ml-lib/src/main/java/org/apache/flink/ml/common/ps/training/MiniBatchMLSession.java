...
public class MiniBatchMLSession<DT> extends MLSessionImpl<DT> {
    ...
    /** Local loss of the last batch. */
    public Double[] localLoss;
    /** Global loss of the last evaluation. */
    ...
    public void initializeState(StateInitializationContext context) throws Exception {
        ...
        lossState =
                context.getOperatorStateStore()
                        .getListState(new ListStateDescriptor<Double>("lossState", Types.DOUBLE));
        Iterator<Double> initLoss = lossState.get().iterator();
        if (initLoss.hasNext()) {
            localLoss = new Double[] {initLoss.next()};
            globalLoss = new Double[] {initLoss.next()};
        } else {
            localLoss = new Double[] {Double.MAX_VALUE};
            globalLoss = new Double[] {Double.MAX_VALUE};
        }
    }
