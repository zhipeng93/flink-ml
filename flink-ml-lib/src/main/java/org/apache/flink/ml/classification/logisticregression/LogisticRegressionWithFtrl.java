...
public class LogisticRegressionWithFtrl
    ...
    private static final Logger LOG = LoggerFactory.getLogger(LogisticRegressionWithFtrl.class);
    ...
    public LogisticRegressionModel fit(Table... inputs) {
        ...
        IterationStageList<MiniBatchMLSession<LabeledPointWithWeight>> iterationStages =
                new IterationStageList<>(mlSession)
                        ...
                        .addStage(
                                new AllReduceStage<>(
                                        (SerializableSupplier<Double[]>) () -> mlSession.localLoss,
                                        (SerializableConsumer<Double[]>)
                                                x -> mlSession.globalLoss = x,
                                        (ReduceFunction<Double[]>)
                                                LogisticRegressionWithFtrl::sumDoubleArray,
                        ...
                        .setTerminationCriteria(
                                (SerializableFunction<
                                                ...
                                                Boolean>)
                                        o -> {
                                            if ((o.iterationId - 1) % getEvaluateInterval() == 0) {
                                                LOG.info(
                                                        "Loss at iteration-"
                                                                + (o.iterationId - 1)
                                                                + " is: "
                                            ...
                                            return o.iterationId >= getMaxIter()
                                                    || o.globalLoss[0] < getTol();
                                        });
