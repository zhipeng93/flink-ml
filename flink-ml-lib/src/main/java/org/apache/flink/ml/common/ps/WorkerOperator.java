...
public class WorkerOperator<DT, SessionT extends MLSession>
    ...
    private <V> void processAllReduceFeedback(AllReduceStage<V> stage, Message message)
            throws IOException {
        stage.recvBuf.accept(message.getValues(stage.typeSerializer));
    }

    private <V> void processReduceScatterFeedback(ReduceScatterStage<V> stage, Message message)
            throws IOException {
        stage.recvBuf.accept(message.getValues(stage.typeSerializer));
    }
    ...
    private <V> int processIterationStages(
        ...
        while (true) {
            ...
            if (stage instanceof PullStage) {
                PullStage pullStage = ((PullStage) stage);
                serverAgent.pull(pullStage.keysSupplier.get(), nextStageToExecute);
                ...
                if (iterationId % reduceScatterStage.executionInterval == 0) {
                    // if (reduceScatterStage.recvCounts == null) {
                    //    int allReduceLength = reduceScatterStage.sendBuf.get().length;
                    //    Preconditions.checkState(allReduceLength % numWorkers == 0, "The length of
                    // reduce scatter cannot be divided by numWorkers.");
                    //    reduceScatterStage.recvCounts = new int[numWorkers];
                    //    Arrays.fill( reduceScatterStage.recvCounts, allReduceLength / numWorkers);
                    // }
                    int[] cnt = reduceScatterStage.recvCounts;
                    int totalLength = Arrays.stream(cnt).sum();
                    Preconditions.checkState(
                            totalLength == reduceScatterStage.sendBuf.get().length,
                            "Illegal recvCounts for ReduceScatter stage.");
                    serverAgent.allReduce(
                ...
                serverAgent.push(
                        pushStage.keysSupplier.get(),
                        pushStage.valuesSupplier.get(),
                        nextStageToExecute);
                nextStageToExecute++;
