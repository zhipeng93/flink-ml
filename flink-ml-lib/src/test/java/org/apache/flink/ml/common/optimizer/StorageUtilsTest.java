...
public class StorageUtilsTest {
    ...
    public void testDenseLongVector() {
        DenseLongVector denseLongVector = new DenseLongVector(new long[] {1L, 2L, 4L});
        byte[] bytes = new byte[32];
        StorageUtils.writeToBytes(denseLongVector, bytes, 2);
        DenseLongVector returnedDenseLongVector = StorageUtils.readFromBytes(bytes, 2);
    ...
    public void testDenseDoubleVector() {
        DenseDoubleVector denseDoubleVector = new DenseDoubleVector(new double[] {1.1, 1.2, 1.3});
        byte[] bytes = new byte[32];
        StorageUtils.writeToBytes(denseDoubleVector, bytes, 2);
        DenseDoubleVector returnedDenseVector = StorageUtils.readFromBytes(bytes, 2);
