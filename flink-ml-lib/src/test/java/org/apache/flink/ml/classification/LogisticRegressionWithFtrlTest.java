...
public class LogisticRegressionWithFtrlTest {
    ...
    @Test
    @SuppressWarnings("unchecked")
    public void testEarlyStopByTol2() throws Exception {
        LogisticRegressionWithFtrl logisticRegressionWithFtrl =
                new LogisticRegressionWithFtrl()
                        .setMaxIter(MAX_ITER)
                        .setNumServers(NUM_SERVERS)
                        .setEvaluateInterval(10)
                        .setTol(0.3);
        LogisticRegressionModel model = logisticRegressionWithFtrl.fit(trainTable);
        List<LogisticRegressionModelDataSegment> modelData =
                IteratorUtils.toList(
        ...
        assertArrayEquals(new double[] {0.30, -0.59, -0.57, -0.40}, collectedCoefficient, 0.1);
    }
